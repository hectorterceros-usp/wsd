# -*- coding: utf-8 -*-
# pego em https://www.inf.ed.ac.uk/teaching/courses/fnlp/Tutorials/7_WSD/wsd_code.py
# foi alterado para compatibilidade com python3
...
def demo(word='hard'):
    print('Word for disambiguation:   {}'.format(word))
    print('Most Frequent')
    sense_fd = nltk.FreqDist([i.senses[0] for i in senseval.instances('{}.pos'.format(word))])
    # most_frequent_hard_sense = hard_sense_fd.keys()[0]
    most_frequent_sense = sense_fd.most_common()[0][0]
    frequency_sense_baseline = sense_fd.freq(most_frequent_sense)
    print('Accuracy: {}'.format(frequency_sense_baseline))
    print()
    print( "NB, with features based on 300 most frequent context words")
    wst_classifier(NaiveBayesClassifier.train, '{}.pos'.format(word), wsd_word_features)
    print()
    print( "NB, with features based word + pos in 6 word window")
    wst_classifier(NaiveBayesClassifier.train, '{}.pos'.format(word), wsd_context_features)
    print('------------------------------------')
    print()
...
# most_frequent_hard_sense = hard_sense_fd.keys()[0]
most_frequent_hard_sense = hard_sense_fd.most_common()[0][0]
frequency_hard_sense_baseline = hard_sense_fd.freq(most_frequent_hard_sense)
